...
(function () {
    ...
    function buildTeamList(response) {
        ...
        var $alphabeticallySortedTeams = response.sort(function (a, b) {
            ...
            return $("<option/>").attr("data-team-id", team.team_id).text("" + team.team_name);
        });

        $("body").prepend($("<p/>").attr("id", "teamsSection").text("Team: ").append($("<select/>").attr("id", "teams").append($alphabeticallySortedTeams)));
    ...
    function buildGameList(response) {
        ...
        var $dateSortedGames = response.sort(function (a, b) {
            ...
            var isHomeTeam = game.localteam_id == window.soccer.teamIdForResults,
                ...
                $option = $("<option/>").attr("data-game-id", game.id).text(printPrettyDate(game.formatted_date) + ": " + firstTeamName + " vs. " + secondTeamName);
        ...
        $("#teamsSection").after($("<p/>").attr("id", "gamesSection").text("Game: ").append($("<select/>").attr("id", "games").append($dateSortedGames)));
    ...
    function buildTable(gameCommentaryJson) {
        var teamIdForResults = soccer.teamIdForResults,
            ...
            firstGoalEvent = gameStatsJson.events.find(function (event) {
            return event.type == "goal";
        }),
            hadFirstGoal = !!firstGoalEvent && firstGoalEvent.team == (isHomeTeam ? "localteam" : "visitorteam"),
            saves = gameCommentaryStatsJson.saves,
            numOfUnsuccessfulPks = gameStatsJson.events.filter(function (event) {
            return event.type == "pen miss" && event.team == (isHomeTeam ? "visitorteam" : "localteam");
        }).length,
        ...
        var gameResultPoints = gameResult == "W" ? 5 : gameResult == "D" ? 2 : 0,
            ...
            goalsAllowedPoints = goalsAllowed == 0 ? 4 : goalsAllowed == 1 ? 2 : goalsAllowed == 2 ? 0 : goalsAllowed == 3 ? -2 : -4,
            ...
            totalPoints = gameResultPoints + goalsScoredPoints + goalsAllowedPoints + shotsOnGoalPoints + hadFirstGoalPoints + savesPoints + yellowCardsPoints + redCardsPoints;

        var $table = $("<table/>").attr("id", "results").append($("<tr/>").append($("<th/>").text("Category")).append($("<th/>").text("Game Stats")).append($("<th/>").text("Points Awarded"))).append($("<tr/>").append($("<td/>").text("Result")).append($("<td/>").text(gameResult)).append($("<td/>").text(gameResultPoints))).append($("<tr/>").append($("<td/>").text("Goals Scored")).append($("<td/>").text(goalsScored)).append($("<td/>").text(goalsScoredPoints))).append($("<tr/>").append($("<td/>").text("Goals Allowed")).append($("<td/>").text(goalsAllowed)).append($("<td/>").text(goalsAllowedPoints))).append($("<tr/>").append($("<td/>").text("Shots on Goal")).append($("<td/>").text(shotsOnGoal)).append($("<td/>").text(shotsOnGoalPoints))).append($("<tr/>").append($("<td/>").text("First Goal?")).append($("<td/>").text(hadFirstGoal ? "yes" : "no")).append($("<td/>").text(hadFirstGoalPoints))).append($("<tr/>").append($("<td/>").text("Saves")).append($("<td/>").text(saves)).append($("<td/>").text(savesPoints))).append($("<tr/>").append($("<td/>").text("PK Saves")).append($("<td/>").attr("id", "pkSaves").text(numOfUnsuccessfulPks ? "" : 0)).append($("<td/>").attr("id", "pkSavesPoints").text(numOfUnsuccessfulPks ? "???" : 0))).append($("<tr/>").append($("<td/>").text("Yellow Cards")).append($("<td/>").text(yellowCards)).append($("<td/>").text(yellowCardsPoints))).append($("<tr/>").append($("<td/>").text("Red Cards")).append($("<td/>").text(redCards)).append($("<td/>").text(redCardsPoints)));

        $("p").last().after($table);

        if (numOfUnsuccessfulPks) {
            var pkSavesCommentary = gameCommentaryJson.comments.filter(function (comment) {
    ...
    function showTotal(totalPoints) {
        $("#results").after($("<p/>").attr("id", "total").addClass("code").text("Total points: " + totalPoints));
    }
